...
get 메서드
- document.getElementById : HTML에서 고유한 ID를 이용하여 요소를 찾는다.
- document.getElementsByClassName : HTML에서 클래스 명으로 요소를 찾는다.
- document.getElementsByTagName : HTML에서 태그명으로 요소를 찾는다.

쿼리 메서드
- document.querySelector : CSS 선택자를 사용하여 HTML 내 첫 번째 요소를 찾는다.
- document.querySelectorAll : CSS 선택자를 사용하여 HTML 내 일치하는 모든 요소를 찾는다.
*/
...
document.getElementById("title").innerHTML = "<h1>프론트엔드@@@@<h1/>" // 태그도 같이 적용 가능.
...
let hello3 = document.createElement("p"); //ex. createElement
hello3.textContent = "Hello";
document.querySelector("body").appendChild(hello3); //ex. appendChild
...
// children
let ui = document.querySelector("#list");
