...
console.log(x+y);
console.log(x/y);
console.log(x%y); // (나누고 남은 숫자)

/*
증감 연산자
 ++x : 전위 증가(x에 1을 증가시킨 후에 명령문 수행)
 x++ : 후위 증가(현재 명령문을 수행 후 x에 1증가)
 --x : 전위 감소(x에 1을 감소시킨 후에 명령문 수행)
 x-- : 소위 감소(현재 명령문을 수행 후 x에 1감소)
*/
...
논리 연산자
- 논리 연산자는 boolean값만 다룰 수 있지만 자바스크립트에서만 모든 타입의 값을 받을 수 있고, 결과 역시 모든 타입이 될 수 있다.
- null, 0, NaN(Not a Number), 빈 문자열(""), undefined, false로 간주한다.
...
단축 평가
- AND 연산자에서 앞쪽 피연산자가 false일때,
  OR 연산에서 앞쪽 피연산자가 true일 때 나머지 피연산자의 평가를 생략하고 그대로 반환한다.
*/
