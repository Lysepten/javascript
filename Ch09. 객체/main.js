...
- 객체가 가지고 있는 값을 속성(프로퍼티)이라고 하며 함수인 경우는 메서드(파라미터)라고 한다.
- 속성(프로퍼티)은 기본 타입이 될 수도 있고, 메서드를 가질 수 있고, 배열, 객체를 프로퍼티로 가질 수 있다.
*/
const obj = {
    // name이 키(Key)
    // '신규섭'이 값(Value)
...
// 구조 분해 할당을 사용하여 개별 변수에 담을 때(객체 순서 상관X)
const {age, name, job} = temp;
...
- 같은 유형의 객체를 여러번 생성하면 번거롭고 가독성이 떨어지는데 팩토리 함수를 사용하여 해결할 수 있다.
*/
...
// 참조 타입(객체 타입) 복사
// 객체를 다른 변수에 할당하면 참조값이 할당 되기 때문에 할당 받은 변수에서 속성을 변경하면 값의 원형이 변경된다.
let obj1 = {name: '신규섭', age: 20};
let obj2 = obj1;
obj2.age = 30;
console.log(obj1);
console.log(obj2);
...
// Call By Refernce: 참조 타입(객체 타입) 호출1
// 값을 가르키는 참조값을 people에 할당 했기 때문에 people의 속성을 변경하면 callName의 속성 값도 함께 변경이 된다.
function change2(people) {
    people.name = '홍길동';
}
let callName2 = { name: "신규섭"};
change2(callName2);
console.log(callName2);

/*
전개 구문
- 객체안의 속성(프로퍼티)를 펼치는 방식으로 할당할 수 있다.
- ... 표기법을 사용한다.
*/
const obj3 = {
    name: '신규섭',
